#!/bin/bash
# Rocket Routers Firmware E v3.11.5 — WSL Build Script
# Run from WSL: bash /mnt/d/Rocket-Routers-Website/firmware-build/build-v3.11.5.sh
#
# Reboot-loop fix build. One change in this build, on top of v3.11.4:
#
#   1. First-boot reboot loop (router stuck cycling LEDs after flash/reset).
#      Leading theory, confirmed consistent with live evidence gathered on a
#      physical unit:
#        - dmesg shows: mtk-wdt 1001c000.watchdog: Watchdog enabled
#          (timeout=31 sec, nowayout=0)
#        - 97-rocket-modem's AT-configuration block (usbnet set + APN
#          CGDCONT) ran in the FOREGROUND with up to 5x5s + 5x5s = ~50s of
#          worst-case retry sleeps, plus a 12s initial GPIO settle sleep —
#          ~62s worst case, all blocking, all on first boot only.
#        - 99-rocket-mwan3 (runs immediately after) adds its own ~30s
#          modem-count detection wait on top of that.
#        - Combined first-boot foreground time could exceed 60-90+ seconds —
#          well past the 31s watchdog window.
#        - Zero kernel oops/panic/OOM in logread, empty /sys/fs/pstore, and a
#          different LED failure point on each reboot-loop attempt — all
#          consistent with a hardware watchdog hard-reset (no chance to log
#          anything), not a software crash.
#      Fix: backgrounded the entire AT-retry block in 97-rocket-modem (now
#      runs in a subshell with `&`) so the first-boot script returns in
#      ~12-14s instead of up to ~62s. The AT configuration still runs to
#      completion and still logs via logger — it's just no longer on the
#      boot-critical blocking path. Mirrors the existing non-blocking pattern
#      already used in 92-rocket-pin-modem and the APN hotplug script.
#
#      NOTE: this is the leading theory, not a confirmed root cause — no
#      crash dump or panic trace was available to prove it directly (that's
#      exactly what a hard watchdog reset looks like). Flash this and watch
#      first boot closely; if the loop recurs, the next thing to check is
#      99-rocket-mwan3's own 30s modem-detection wait loop.
#
...
echo "  # Most important check: did first boot complete WITHOUT the reboot loop?"
echo "  # Watch the LEDs through the flash + first reboot. If it boots clean first"
echo "  # try, that is itself strong evidence for/against the watchdog theory."
echo "  uptime"
echo "  # Confirm the backgrounded AT config actually ran and completed:"
echo "  logread | grep rocket-modem-defaults"
echo "  # Confirm modem still configured correctly despite running in background:"
echo "  gsmctl -A 'AT+QCFG=\"usbnet\"'"
echo "  gsmctl -A 'AT+CGDCONT?'"
echo "  # Re-check the v3.11.4 fixes are still intact (no regressions):"
...
echo ""
echo -e "${YELLOW}${BOLD}NOTE:${RESET} if the reboot loop still recurs on this build, the next"
echo "suspect is 99-rocket-mwan3's own ~30s modem-count detection wait loop —"
echo "not yet touched in this fix."
