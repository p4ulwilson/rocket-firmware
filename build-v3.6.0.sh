#!/bin/bash
# Rocket Routers Firmware E v3.6.0 — WSL Build Script
# Run from WSL: bash /mnt/d/Rocket-Routers-Website/firmware-build/build-v3.6.0.sh
#
# Changes in this build vs v3.5.1:
#   - Dashboard: mesh join/leave now calls ifup/ifdown ygg0 for real
#   - Dashboard: overview tab forces page reload via timestamp (?_=Date.now())
#   - Matrix foundation: hotplug block script auto-mounts SSD + starts conduit
#   - Matrix foundation: conduit procd init script (graceful no-op if no SSD)
#   - Cloudflare Tunnel: cloudflared procd init script (graceful no-op if no SSD)
#   - Both services: auto-start on SSD plug-in, auto-stop on SSD removal
#   - Both services: respawn enabled via procd
#   - Cloudflared START=98, conduit START=99 (cloudflared up first)
#   - Works for all ISPs incl Three UK CGNAT — no incoming ports needed
#
...
# NEW v3.6.0 — SSD block hotplug: auto-mount + start conduit/cloudflared
...
# NEW v3.6.0 — conduit + cloudflared procd init scripts
...
METADATA="$FIRMWARE_BUILD/rocket-vE-v3.6.0-metadata.config"
if [ ! -f "$METADATA" ]; then
