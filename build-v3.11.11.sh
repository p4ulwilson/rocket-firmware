#!/bin/bash
# Mycelium Mesh RocketRouters v3.11.11 — WSL Build Script
# Run from WSL: bash /mnt/d/Rocket-Routers-Website/firmware-build/build-v3.11.11.sh
#
# One change in this build, on top of v3.11.10 — fixes the stale dashboard
# version badge ("mycelium build 3.10.1") Paul spotted right after flashing
# v3.11.9:
#
#   1. rocket-dashboard.cgi read /etc/rocket/mycelium_version on the router,
#      but only ever CREATED that file if missing — never overwrote an
#      existing one — and the fallback value used to create it was the
#      literal string "3.10.1", hardcoded when the script was first written,
#      with zero connection to whatever VERSION a build actually shipped.
#      So on every single fresh flash, the very first time the dashboard
#      loaded and found the file missing, it wrote "3.10.1" straight to
#      disk — 100% reproducible, completely independent of which real
#      firmware version was running. NOT an overlay/persistence issue and
#      NOT something a plain rebuild fixes on its own, because the broken
#      logic is identical from build to build until the source itself
#      changes — v3.11.10 still has this exact bug, badge and all.
#      Fix: changed the literal fallback in rocket-dashboard.cgi to the
#      same __ROCKET_FW_VERSION__ placeholder already used for
#      /etc/rocket-routers-version and /etc/banner, and added a sed-stamp
#      step (below, where www/cgi-bin/rocket gets staged) so it gets
#      filled in with the real ${VERSION} at build time, every build,
#      automatically — same pattern as #19/#32, just applied to the one
#      file that was missed.
#      NOTE: this only fixes the badge for FRESH flashes (no existing
#      /etc/rocket/mycelium_version) and any router where the file is
#      empty/missing. If a router already has a stale value sitting in
#      that file from before, it'll keep showing the old number until
#      that file is cleared — over SSH: `rm /etc/rocket/mycelium_version`
#      then reload the dashboard.
#
# Carries forward v3.11.10's GPIO power-cycle fix for the reboot loop
# unchanged (still NOT YET CONFIRMED — pending Paul's physical test of
# v3.11.10 first boot + subsequent plain reboots), plus all v3.11.9 and
# earlier fixes: donate-gate removal (joinMesh() unlocks everything),
# Cake/SQM dashboard fixes, stale version banner fix, set -e / uci-delete
# fix, modem-count detection window, mwan3 policy names, wwan2 dhcp/usb1
...
# This build's one real change: the mycelium-build version badge fallback
# in rocket-dashboard.cgi is now __ROCKET_FW_VERSION__ instead of a stale
# hardcoded "3.10.1" — sed-stamp it the same way 99-rocket-mwan3 and
# 98-rocket-branding already get stamped above, so the staged copy carries
# the real version, not the literal placeholder.
if [ -f "$FIRMWARE_BUILD/rocket-dashboard.cgi" ]; then
