#!/bin/bash
# Rocket Routers Firmware E v3.7.0 — WSL Build Script
# Run from WSL: bash /mnt/d/Rocket-Routers-Website/firmware-build/build-v3.6.1.sh
#
# Changes in this build vs v3.6.1:
#   - User accounts: Matrix registration + login (@you:rocketrouters.co.uk)
#   - Account tab: dedicated tab replacing header modal
#   - Unique per-device invite code: auto-generated on first boot (init-conduit)
#   - Owner can reveal + change invite code from Account tab
#   - Chat M_FORBIDDEN fix: user token used for room reads when logged in
#   - Inline video player in chat (no download required)
#   - jfif/avif file type support in chat uploads
#   - Chat DOM diff rendering: video/media elements survive poll cycles
#   - WebRTC Live tab: peer-to-peer video calls over mesh
#   - Camera/mic test in Live tab
#   - LAN subnet auto-grant (family on same WiFi get mesh+donate flags)
#   - Live tab hash routing fixed (no redirect to Earn on refresh)
#   - File serving: static path + HTTP range requests for video seeking
#
...
# NEW v3.6.1 — SSD block hotplug: auto-mount + start conduit/cloudflared
...
# NEW v3.6.1 — conduit + cloudflared procd init scripts
# NEW v3.7.0 — init-conduit: auto-generates unique invite code on first boot
...
METADATA="$FIRMWARE_BUILD/rocket-vE-v3.7.0-metadata.config"
if [ ! -f "$METADATA" ]; then
...
echo "  curl -s http://192.168.1.1/_matrix/client/versions  # → Matrix API"
echo "  /etc/init.d/conduit status                   # → running (if SSD present)"
echo "  /etc/init.d/cloudflared status               # → running (if SSD present)"
echo "  curl https://matrix.rocketrouters.co.uk/_matrix/client/versions  # → global"
