#!/bin/bash
# Rocket Routers Firmware E v3.8.1 — WSL Build Script
# Run from WSL: bash /mnt/d/Rocket-Routers-Website/firmware-build/build-v3.8.1.sh
#
# Changes in this build vs v3.8.0:
#   - Claim ownership: shows on logged-out reg page when no owner set (gold banner)
#   - Claim ownership: auto-triggers after registration if no owner set
#   - Delete account: pre-cleanup CGI call fires before UIAA (owner file clears correctly)
#   - user_delete_account added to _SKIP_AUTH so CGI cleanup works without cookie session
#   - Delete warning: username permanently retired + videos go anonymous + chat lost
#   - Registration warning: username is permanent, cannot be reused after deletion
#
...
METADATA="$FIRMWARE_BUILD/rocket-vE-v3.8.1-metadata.config"
if [ ! -f "$METADATA" ]; then
