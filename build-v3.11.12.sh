#!/bin/bash
# Mycelium Mesh RocketRouters v3.11.12 — WSL Build Script
# Run from WSL: bash /mnt/d/Rocket-Routers-Website/firmware-build/build-v3.11.12.sh
#
# One change in this build — a revision of v3.11.10's reboot-loop fix,
# based on what Paul actually observed flashing v3.11.10 live:
#
#   1. v3.11.10 still loops, but differently than before: ~11s of normal
#      boot LEDs, then a solid ~120s on red, then a hard reset. That's a
#      NEW failure shape — long and consistent — not the old "loops
#      forever until unplug" pattern.
#      Likely cause: v3.11.10's rc.local ran a full GPIO low->high pulse
#      (genuine power-off then power-on) on EVERY boot, not just first
#      boot. That means every single warm reboot now forces both 5G
#      modems through a real cold power-on, baseband and all. A Quectel
#      RM500U-EA cold-booting from a dead stop can realistically take
#      60-120+ seconds to fully attach — and with two modems doing that
#      every boot, ~120s before the hardware watchdog (31s timeout, but
#      only starts counting once something actually stops feeding it)
#      finally gives up lines up uncomfortably well.
#      In other words: the v3.11.10 fix likely traded an occasional
#      first-boot/post-flash wedge for a near-guaranteed long hang on
#      EVERY warm boot.
#      NOT YET CONFIRMED — same caveat as v3.11.10's own note. Best
#      evidenced theory given the timing, not a proven cause.
#      Fix: rc.local goes back to a plain, fast assert-HIGH on every
#      ordinary boot — exactly what every version before v3.11.10 did,
#      and which never produced this ~120s hang. The real destructive
#      low->high power-cycle pulse stays, but ONLY in the first-boot
#      inline block in 97-rocket-modem (where it's genuinely needed right
#      after a fresh flash). Ordinary warm reboots no longer force a full
#      modem cold-boot.
#      If this build still loops: the cold-boot-duration theory is wrong
#      (or incomplete), and the original wedged-modem-needs-a-real-power-
#      cycle theory from v3.11.10 may need to come back in a smarter
#      form — e.g. only force the destructive pulse after detecting the
#      modem is actually unresponsive, rather than on every single boot.
#
# Second, separate fix in this build — "interface up, but no internet":
#   Diagnosed live on the router via mwan3 status/interfaces, ip route show,
#   and AT commands. Root cause: neither network.wwan nor network.wwan2 ever
#   set defaultroute='0' or peerdns='0'. mwan3 is supposed to own ALL real
#   traffic routing through its own per-policy tables, but without those two
#   options each interface's plain DHCP client was free to shove its own
#   default route into the MAIN routing table and overwrite /etc/resolv.conf
#   — and whichever modem's lease renewed last simply won that one slot,
#   regardless of which modem mwan3 itself considered online. Confirmed
#   live: `ip route show` had the only default route pointing at the modem
#   mwan3 called OFFLINE (wwan2/usb1), while the modem mwan3 called ONLINE
#   (wwan/usb0) had no route in the main table at all. That's exactly the
#   "mwan3 says online, ping still dead" symptom.
#   Fix: added defaultroute='0' and peerdns='0' to both network.wwan
#   (97-rocket-modem) and network.wwan2 (99-rocket-mwan3), so the main
#   table stays out of mwan3's way entirely. This one is source-confirmed
#   against live router output, not a theory — should genuinely fix the
#   no-internet symptom.
#
# Carries forward v3.11.11's dashboard version-badge fix and all v3.11.9
# and earlier fixes unchanged: donate-gate removal (joinMesh() unlocks
# everything), Cake/SQM dashboard fixes, stale version banner fix,
# set -e / uci-delete fix, modem-count detection window, mwan3 policy
# names, wwan2 dhcp/usb1 rework, Cake default-off.
#
...
# the real v3.11.12, not the literal placeholder.
if [ -f "$FIRMWARE_BUILD/rocket-dashboard.cgi" ]; then
