#!/bin/bash
# Rocket Routers Firmware E v3.8.0 — WSL Build Script
# Run from WSL: bash /mnt/d/Rocket-Routers-Website/firmware-build/build-v3.8.0.sh
#
# Changes in this build vs v3.7.0:
#   - Profile picture upload: openssl enc -base64 -d -A (base64 binary not on router)
#   - Avatar system: 12 preset avatars, stored as .avt index in /mnt/ssd/rr-profiles/
#   - Gender picker: Lady/Gent at registration, sets default avatar (index 4=Lady, 10=Gent)
#   - Delete account: UIAA two-step with password (conduwuit has no Synapse admin deactivate)
#   - Claim ownership: /etc/rocket/owner claim endpoint for unclaimed nodes
#   - Live chat: send via URL params, poll handles {msgs:[]} wrapper, empty msg filtering
#   - Live test UI: correct element IDs for video/audio/preview/bar
#   - Mic level bar: real-time audio analyser in live test
#   - Profile pic: canvas crop/resize 96×96 @ 0.72 quality before upload
#   - Videos: paul/anon videos cleared from /mnt/ssd/rr-video/
#
...
# init-conduit: auto-generates unique invite code on first boot
...
METADATA="$FIRMWARE_BUILD/rocket-vE-v3.8.0-metadata.config"
if [ ! -f "$METADATA" ]; then
...
echo "  cat /etc/rocket/owner                        # → p4ul"
