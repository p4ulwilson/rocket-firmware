#!/bin/bash
# Rocket Routers Firmware E v3.5.0 — WSL Build Script
# Run from WSL: bash /mnt/d/Rocket-Routers-Website/firmware-build/build-v3.5.0.sh
#
# Changes in this build vs v3.4.2:
#   - Dashboard: v3.5.0 — major feature release
#   - Auth: login page with session cookie — dashboard password protected per router
#   - Auth: SHA-256 hashed credentials stored in /etc/rr_auth (written by credential tool)
#   - Auth: logout button in header, session cleared on reboot
#   - Landing: 192.168.1.1 → Rocket Routers dashboard (not LuCI) via www/index.html
#   - Matrix: slow-reveal phrase mode — phrase chars appear one by one, randoms at full speed
#   - Matrix: new phrase "INTELLIGENCE BELONGS TO EVERYONE"
#   - Claude tab: glass cards — matrix fully visible through all cards
#   - WiFi: channel optimiser — scans neighbours, scores ch1/6/11, one-tap apply
#   - DNS: Family Shield (Cloudflare 1.1.1.3) — CSAM + adult content blocking, all devices
#   - Ad block: one-tap Hagezi light blocklist (~40k domains) via dnsmasq, every device
#   - Protect tab: IWF + CEOP reporting links, live protection status, how-it-works card
#   - Expert: nightly reboot toggle, signal sparkline, DNS switcher incl. Family Shield
#   - Earn: ad block card with bandwidth stats, mesh-join matrix canvas
#
...
if [ -f "$FIRMWARE_BUILD/rocket-dashboard.cgi" ]; then
    ...
    echo "  staged: www/cgi-bin/rocket (dashboard)"
else
    echo -e "  ${YELLOW}SKIP: rocket-dashboard.cgi not found${RESET}"
fi
...
# Default landing page — http://192.168.1.1/ redirects to Rocket Dashboard
# Without this, OpenWrt serves the LuCI login as the root page.
...
METADATA="$FIRMWARE_BUILD/rocket-vE-v3.5.0-metadata.config"
if [ ! -f "$METADATA" ]; then
...
# Belt + suspenders: ensure critical packages are explicitly in .config
...
echo "  iwinfo                                        # → tri-band radios up"
echo "  curl -s http://192.168.1.1/ | head -5         # → LuCI serving HTML"
echo "  logread | grep rocket                         # → no error 127"
echo "  sms_tool -d /dev/ttyUSB2 at 'AT+CGDCONT?'   # → three.co.uk"
echo "  sms_tool -d /dev/ttyUSB2 at 'AT+QNWPREFCFG=\"lte_band\"'  # → 1:3:20:28"
echo "  ip -6 addr show dev ygg0 | grep 'inet6 2'    # → stable Ygg address"
echo "  hostname                                      # → RocketPlus-E"
