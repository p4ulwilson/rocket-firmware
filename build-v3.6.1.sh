#!/bin/bash
# Rocket Routers Firmware E v3.6.1 — WSL Build Script
# Run from WSL: bash /mnt/d/Rocket-Routers-Website/firmware-build/build-v3.6.1.sh
#
# Changes in this build vs v3.6.0:
#   - Hotplug: config.yml auto-written from SSD credentials on every boot/plug-in
#   - Cloudflared: no manual config restore needed after sysupgrade ever again
#   - Credentials cert.pem + JSON live on SSD, survive all future flashes
#   
#   
#   
#   
...
METADATA="$FIRMWARE_BUILD/rocket-vE-v3.6.1-metadata.config"
if [ ! -f "$METADATA" ]; then
