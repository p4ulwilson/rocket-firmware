#!/bin/bash
# Mycelium Mesh RocketRouters v3.11.9 — WSL Build Script
# Run from WSL: bash /mnt/d/Rocket-Routers-Website/firmware-build/build-v3.11.9.sh
#
# One change in this build, on top of v3.11.8:
#
#   1. Donate-gate removed from the dashboard. joinMesh() now internally
#      calls donateMesh(true) (and leaveMesh() calls undonate()), so the
#      old separate "donate your 75%" step is gone — joining the mesh is
#      now the only unlock action needed. Updated everywhere it appeared:
#      the t-ssd tab's standalone donate card was removed (folded into the
#      single "JOIN THE MESH" card), and the Claude/Video/Chat tabs'
#      locked-feature copy and buttons were changed from
#      donateMesh()/"Donate my 75%..." to joinMesh()/"Join the Mesh...".
#      rocket-dashboard.cgi was edited Jun 18 01:06 — AFTER the v3.11.8
#      .bin was already built (Jun 17 21:19) — so v3.11.8.bin does NOT
#      contain this change. This build's only job is to bake it in.
#
...
#
# Original v3.11.8 changelog follows unchanged (Cake/SQM dashboard fixes +
# stale version banner fix):
#
...
#      regardless of what was actually flashed. 99-rocket-mwan3 now writes
#      a __ROCKET_FW_VERSION__ placeholder; this build script stamps the
#      real ${VERSION} into it at staging time (see step 3 below).
#
# Carries forward all v3.11.7 and earlier fixes unchanged (set -e /
# uci-delete fix, modem-count detection window, reboot-loop backgrounding,
# mwan3 policy names, wwan2 dhcp/usb1 rework, Cake default-off).
# Reminder: the reboot loop itself is STILL OPEN/unconfirmed long-term —
# not touched in this build, watch uptime as before.
#
...
# Version banner placeholder stamping (carried forward from v3.11.8 fix) —
# keeps /etc/rocket-routers-version and the branding system description
...
# THIS is the actual point of this build: rocket-dashboard.cgi now has the
# donate-gate removed (joinMesh() unlocks everything; donateMesh() is
# internal-only). v3.11.8.bin was built before this edit landed, so it's
# missing here — that's the one and only thing v3.11.9 exists to fix.
if [ -f "$FIRMWARE_BUILD/rocket-dashboard.cgi" ]; then
...
echo "  # expect: Rocket Routers Firmware A v${VERSION} (or current branding string)"
echo "  # Confirm the actual point of this build on the dashboard itself:"
echo "  # open the Claude / Video / Chat tabs while NOT in the mesh — buttons"
echo "  # should read 'Join the Mesh and unlock...' not 'Donate my 75%...'"
...
echo "in this build. WPS is also NOT in this build — that's reserved for a"
echo "future build tied to the physical mesh button. Watch uptime as before."
