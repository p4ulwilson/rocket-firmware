#!/bin/bash
# Rocket Routers Firmware E v3.8.2 — WSL Build Script
# Run from WSL: bash /mnt/d/Rocket-Routers-Website/firmware-build/build-v3.8.2.sh
#
# Changes in this build vs v3.8.1:
#   - tcpdump + libpcap baked in (apk install had ABI mismatch — now compiled from source)
#   - Expert tab: full nmap switch suite (evasion, scan types, host discovery, timing T0-T5)
#   - Expert tab: packet capture countdown + busybox-compatible sleep/kill (no timeout cmd)
#   - Expert tab: vnstat hourly display fixed (pipe-separated format parsed correctly)
#   - Nav: Why + Mycelium links added to all pages that were missing them
#   - Rocket Starter removed from all pages + rocket-starter.html deleted
#
...
METADATA="$FIRMWARE_BUILD/rocket-vE-v3.8.2-metadata.config"
if [ ! -f "$METADATA" ]; then
