#!/bin/bash
# Rocket Routers Firmware E v3.4.2 — WSL Build Script
# Run from WSL: bash /mnt/d/Rocket-Routers-Website/firmware-build/build-v3.4.2.sh
#
# Fixes in this build vs v3.4.1:
#   - LuCI: uhttpd + luci-theme-bootstrap + luci-app-qmodem now EXPLICIT in config
#           (v3.4.1 had luci-base but no web server — LuCI was invisible)
#   - LuCI: luci-proto-wireguard + luci-app-wireguard added (fixes "unsupported protocol")
#   - Modem: band preference set to AUTO + LTE bands 1/3/20/28 at boot
#            (RM500U-EA was latching onto weak Band 1 instead of Band 20)
#   - Blocklist: @reboot cron syntax replaced (BusyBox crond doesn't support @reboot)
#   - Hostname: RocketPlus-E branding set at first boot
#
...
METADATA="$FIRMWARE_BUILD/rocket-vE-v3.4.2-metadata.config"
if [ ! -f "$METADATA" ]; then
