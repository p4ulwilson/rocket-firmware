#!/bin/bash
# Rocket Routers Firmware E v3.10.0 — WSL Build Script
# Run from WSL: bash /mnt/d/Rocket-Routers-Website/firmware-build/build-v3.10.0.sh
#
# Changes in this build vs v3.9.2:
#   - Video platform: anonymous video uploads (anon=1 flag, checkbox UI)
#   - Video player: fullscreen fixed (was half-screen due to max-height constraint)
#   - Video player: mute button (🔊/🔇) + volume slider added
#   - Video player: seeking fixed (HTTP Range request support, 206 Partial Content)
#   - Comments: posting fixed (token in POST body — uhttpd strips Authorization header)
#   - Comments: delete fixed (same token-in-body fix + VID_ID extraction bug with cid=)
#   - Comments: vote VID_ID extraction fix (same grep bug as delete)
#   - Login: avatar picker hidden in login mode; error messages mode-specific
#   - Branding: hostname changed from RocketPlus-E → rocketrouters
#   - Branding: dashboard subtitle changed to "RocketRockers"
#   - Branding: firmware version bumped to v3.10.0
#
...
if [ -f "$FIRMWARE_BUILD/banner" ]; then
    cp "$FIRMWARE_BUILD/banner" files/etc/banner
    echo "  staged: etc/banner (Rocket Routers branding)"
else
