#!/bin/bash
# Rocket Routers Firmware E v3.11.4 — WSL Build Script
# Run from WSL: bash /mnt/d/Rocket-Routers-Website/firmware-build/build-v3.11.4.sh
#
# Dual-WAN architecture fix build. Four real bugs fixed in this build:
#
#   1. mwan3 policy names exceeding 15 chars (fixed in v3.11.3 already —
#      carried forward unchanged here).
#
#   2. wwan2 (second modem) could NEVER come up. 99-rocket-mwan3 configured
#      network.wwan2 as proto='modemmanager', but 97-rocket-modem explicitly
#      stops/disables ModemManager at first boot ("Q-modem handles
#      everything"). Confirmed live on a Supreme unit:
#        ubus call network.interface.wwan2 status -> {"errors":[{"code":"NO_DEVICE"}]}
#      The second modem's actual internet connectivity was coming entirely
#      from a QModem-auto-created interface ("4_1") that mwan3 has zero
#      configuration awareness of — meaning dual-WAN load-balancing/failover
#      has never actually worked on shipped dual-modem units, even after the
#      policy-name fix above. Fixed by reconfiguring wwan2 as a plain DHCP
#      client on usb1, mirroring exactly how wwan/usb0 already works, plus:
#        - new /etc/hotplug.d/net/31-wwan2-up (auto ifup wwan2 when usb1
#          appears, mirroring 30-wwan-up for usb0)
#        - wwan2 added to the firewall WAN zone (was never a member of any
#          firewall zone even in the original broken design — a third,
#          previously undiagnosed gap)
#
#   3. Dashboard Cake status/toggle (cake_status / cake_set in
#      rocket-dashboard.cgi) compared sqm.*.interface (a LOGICAL uci name
#      like wan/wwan/wwan2) directly against the REAL kernel device carrying
#      the default route (usb0/usb1/eth1) — these never matched, so the
#      dashboard always fell back to a hardcoded "wwan" guess regardless of
#      which WAN was actually active. Fixed by resolving each sqm section's
#      logical interface to its real l3_device via ubus+jsonfilter before
#      comparing. Also fixed a companion bug found during this fix: cake_set
#      was writing the REAL device name into sqm.*.interface instead of the
#      logical name, silently corrupting the section on every save.
#
#   4. Cake (SQM) was enabled='1' by default on every WAN/cell leg straight
#      out of the box, with download/upload left at 0 (unlimited
#      placeholder) — silently shaping/throttling lines nobody asked to
#      shape. Changed sqm_apply()'s default to enabled='0'; Cake is now an
#      explicit opt-in via the dashboard (cake_set) once real bandwidth
#      numbers are known.
#
# NOTE: this build does NOT address the router reboot-loop issue reported
# separately. No root cause for that was identified during this debugging
# pass — it is not fixed here and should not be assumed fixed by this build.
#
...
# 31-wwan2-up: auto ifup wwan2 when usb1 appears (second modem). NEW in
# v3.11.4 — companion to 30-wwan-up, needed now that wwan2 is a real dhcp
# interface instead of the dead modemmanager proto. Only fires/matters on
# dual-modem (Supreme) units; harmless no-op on single-modem units since
# usb1 never appears there.
...
# Left unchanged in v3.11.4 — once wwan2 directly claims usb1 (mirroring how
# wwan already claims usb0), QModem should stop auto-creating the competing
# "4_1" interface, by the same logic it doesn't auto-create one for the
# already-claimed usb0. This script should become naturally inert rather
# than something that needs editing. Kept staged as a safety net regardless.
...
# Carries this build's cake_status/cake_set logical-vs-real-device fix.
if [ -f "$FIRMWARE_BUILD/rocket-dashboard.cgi" ]; then
...
echo "  # Check second modem now has a real DHCP wwan2 interface (not NO_DEVICE):"
echo "  ubus call network.interface.wwan2 status"
echo "  # Check wwan2 is in the WAN firewall zone:"
echo "  uci show firewall | grep -A3 \"zone\\[.*name='wan'\" "
echo "  # Check mwan3 sees both modems as real members:"
echo "  uci show mwan3 | grep -E 'policy|member|interface'"
echo "  # Check Cake is OFF by default on all legs (must be 0, not 1):"
echo "  uci show sqm | grep enabled"
echo "  # Check Cake status/toggle resolves the correct real device (no more"
echo "  # hardcoded wwan fallback regardless of which WAN is active):"
echo "  curl -s 'http://192.168.1.1/cgi-bin/rocket?cake_status' | python3 -m json.tool"
echo "  # Check default route / which modem is actually carrying traffic:"
echo "  ip route show default"
echo ""
echo -e "${YELLOW}${BOLD}NOTE:${RESET} this build does NOT fix the router reboot-loop issue."
echo "No root cause for that was found during this debugging pass."
