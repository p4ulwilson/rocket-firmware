#!/bin/bash
# Rocket Routers Firmware E v3.10.1 — WSL Build Script
# Run from WSL: bash /mnt/d/Rocket-Routers-Website/firmware-build/build-v3.10.1.sh
#
# Changes in this build vs v3.10.0:
#   - 6GHz radio (radio2/phy0.2-ap0) fixed: hostapd hard-refuses any open/
#     pre-RSNA AP on 6GHz ("Pre-RSNA security methods are not allowed in
#     6 GHz") — it was shipping wide open and silently never coming up.
#   - Dropped 6GHz from EHT320 (320MHz, needs AFC auth consumer APs don't
#     have) to EHT160 + auto channel — confirmed working at full US power.
#   - 98-rocket-wifi (factory first-boot): 6GHz now gets a unique per-unit
#     default WPA3 (SAE) password derived from the LAN MAC address, so it
#     works out of the box. 2.4/5GHz still ship open as before.
#   - Dashboard WiFi-password card (net_wizard_wifi): was setting psk2
#     (WPA2) uniformly across all bands — also forbidden on 6GHz. Now
#     band-aware: sae on 6GHz, psk2 on 2.4/5GHz.
#   - Dashboard firmware badge: now reads a self-bootstrapping
#     /etc/rocket/mycelium_version file instead of a hardcoded string —
#     bump the displayed build number over SSH without redeploying the CGI.
#
