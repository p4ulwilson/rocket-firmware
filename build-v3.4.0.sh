#!/bin/bash
# Rocket Routers Firmware E v3.4.0 — WSL Build Script
# Run from WSL: bash /mnt/d/Rocket-Routers-Website/firmware-build/build-v3.4.0.sh
#
# Fixes in this build vs v3.3.0:
#   - WiFi: removed set -e from 98-rocket-wifi (was killing config silently)
#   - sms-tool: added CONFIG_PACKAGE_sms-tool=y (was missing — all AT commands failed)
#   - pin-modem: fixed infinite procd respawn loop (now oneshot)
#   - Hotplug scripts baked directly into squashfs (not just deployed by uci-defaults)
#
# Build system notes (learned the hard way):
#   - Package manager: APK (not opkg) — pttuan's custom branch
#   - openwrt_one must be commented out in filogic.mk (missing MT7981 BL31/uboot)
...
# ── 2. Strip Windows paths from WSL PATH (parentheses in "Program Files (x86)"
#        break bash when the build system passes PATH to sub-make shells)
# ─────────────────────────────────────────────────────────────────────────────
...
# Disable devices that fail to build (missing MT7981 BL31/uboot artifacts
# in pttuan's branch). Add more entries here if new failures appear.
...
# ── 4. Stage hotplug scripts DIRECTLY into squashfs (belt + suspenders) ───────
echo -e "\n${BOLD}[4/8] Staging hotplug scripts into squashfs...${RESET}"
#
# These are also deployed by 97-rocket-modem at first boot, but baking them
# directly into files/ means they survive even if uci-defaults has a bug.
#
mkdir -p files/etc/hotplug.d/usb
mkdir -p files/etc/hotplug.d/net

# USB hotplug: re-apply APN when modem USB reconnects
if [ -f "$FIRMWARE_BUILD/hotplug-usb-20-quectel-smarty-apn" ]; then
    ...
    echo -e "  ${RED}ERROR: hotplug-usb-20-quectel-smarty-apn not found in firmware-build!${RESET}"
    exit 1
fi

# Net hotplug: ifup wwan when usb0 appears (netifd misses it on boot)
if [ -f "$FIRMWARE_BUILD/hotplug-net-30-wwan-up" ]; then
    ...
    echo -e "  ${RED}ERROR: hotplug-net-30-wwan-up not found in firmware-build!${RESET}"
    exit 1
fi
...
METADATA="$FIRMWARE_BUILD/rocket-vE-v3.4.0-metadata.config"
if [ ! -f "$METADATA" ]; then
    echo -e "${RED}ERROR: $METADATA not found${RESET}"
    exit 1
fi

# Append sms-tool and other v3.4.0 packages to .config
sed -i '/^CONFIG_VERSION_/d' .config
cat "$METADATA" >> .config

# Add sms-tool to .config (belt + suspenders alongside rocket-vE.config)
if ! grep -q "CONFIG_PACKAGE_sms-tool" .config; then
    echo "CONFIG_PACKAGE_sms-tool=y" >> .config
    echo "  added: CONFIG_PACKAGE_sms-tool=y"
fi

echo -e "${GREEN}Version metadata set: Rocket Plus E v${VERSION}${RESET}"
grep "CONFIG_VERSION_NUMBER" .config

# ── 7. Append rocket-vE.config packages ──────────────────────────────────────
echo -e "\n${BOLD}[6b/8] Appending rocket-vE.config packages...${RESET}"
if [ -f "$FIRMWARE_BUILD/rocket-vE.config" ]; then
    # Remove existing package lines to avoid duplicates, then re-add fresh
...
# ── Manual image assembly (Z8803BE build system bug — sysupgrade-tar silently skipped) ──
echo -e "\n${BOLD}[8/8] Manual sysupgrade image assembly...${RESET}"
#
# pttuan's branch has a bug where sysupgrade-tar/Image/Build never fires for
# Z8803BE. The kernel and rootfs .bin files are built, but the sysupgrade.bin
# is never produced. Assemble it manually using sysupgrade-tar.sh.

# The Z8803BE squashfs kernel lives in build_dir, NOT in bin/targets/
# (pttuan branch build system bug — image assembly silently skipped).
# The kernel WITHOUT "initramfs" in the name is the squashfs sysupgrade kernel.
...
if [ ! -f "$KERNEL" ]; then
    ...
    echo "All z8803be files in build_dir:"
...
echo ""
echo -e "${BOLD}Verify binary before flash:${RESET}"
echo "  file $BIN_DEST"
echo "  ls -lh $BIN_DEST"
echo ""
echo -e "${BOLD}Transfer to router:${RESET}"
...
echo ""
echo -e "${BOLD}Post-flash verification (SSH in after reboot):${RESET}"
echo "  iwinfo                                        # → radios showing"
echo "  ping -c 3 8.8.8.8                            # → internet works"
echo "  logread | grep rocket                         # → no error 127"
echo "  sms_tool -d /dev/ttyUSB2 at 'AT+CGDCONT?'   # → three.co.uk"
echo "  ip -6 addr show dev ygg0 | grep 'inet6 2'    # → stable Ygg address"
echo "  ls /etc/hotplug.d/net/                       # → 30-wwan-up present"
echo "  ls /etc/hotplug.d/usb/                       # → 20-quectel-smarty-apn present"
