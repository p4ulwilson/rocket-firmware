#!/bin/bash
# Rocket Routers Firmware E v3.8.3 — WSL Build Script
# Run from WSL: bash /mnt/d/Rocket-Routers-Website/firmware-build/build-v3.8.3.sh
#
# Changes in this build vs v3.8.2:
#   - WORLD FIRST: Live Network Topology Map (D3.js force graph, real-time, drag/hover)
#   - WORLD FIRST: BGP Route Tracer (traceroute -> geolocate -> world map, sat/hybrid view)
#   - WORLD FIRST: AI Packet Analyst (async tcpdump -> Claude Haiku, up to 5min captures)
#   - WORLD FIRST: WiFi Device Scheduler (instant pause / bedtime schedules per device)
#   - d3.min.js bundled at /www/d3.min.js (no CDN dependency, extension-safe)
#   - Async tcpdump capture: non-blocking, live countdown, Stop Early button
#   - Async traceroute: 2s poll, RTT-coloured segments, satellite/hybrid map views
#   - routers.html: World Firsts marketing section added
...
if [ -f "$FIRMWARE_BUILD/d3.min.js" ]; then
    ...
    echo -e "  ${YELLOW}WARN: d3.min.js not found at $FIRMWARE_BUILD/d3.min.js${RESET}"
fi
...
METADATA="$FIRMWARE_BUILD/rocket-vE-v3.8.3-metadata.config"
if [ ! -f "$METADATA" ]; then
