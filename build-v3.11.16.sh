#!/bin/bash
# Mycelium Mesh RocketRouters v3.11.16 — WSL Build Script
# Run from WSL: bash /mnt/d/Rocket-Routers-Website/firmware-build/build-v3.11.16.sh
#
# This build's fix — v3.11.15 also did nothing, here's what changed:
#
#   v3.11.13/14/15 all tried to AUTO-DETECT which modem has no SIM by
#   sending it AT+CPIN? and reading the reply. v3.11.15 added unconditional
#   raw-response logging specifically to settle this, and it did: across
#   multiple full boots (9+ minutes, 10+ checks), qmodem section "2_1"
#   (/dev/ttyUSB3 — the modem with NO SIM fitted) NEVER sent back a real AT
#   reply. Every single check just got the echoed command back, nothing
#   else. Meanwhile section "4_1" (/dev/ttyUSB8 — the modem WITH the SIM)
#   answered correctly every single time ("+CPIN: READY"). The detection
#   method only ever worked on the modem that didn't need fixing — it could
#   never have disabled 2_1, no matter how long it ran.
#
#   There's also direct evidence of a second, separate issue: one logged
#   pass shows a fragment of a DIFFERENT AT command (AT+CGPADDR) bleeding
#   into the read of 4_1 — something else (qmodem's own AT daemon) is
#   talking to these same ports at the same time we are. qmodem's own
#   README explicitly warns that concurrent AT command access causes
#   exactly this: garbled/incomplete replies, timeouts, disconnects.
#
#   Fix: stop asking the modem anything. 94b-rocket-sim-check now hardcodes
#   qmodem section "2_1" as the no-SIM modem (confirmed empirically,
#   repeatedly) and force-disables it every 60s forever — no AT commands,
#   no detection, nothing that can fail to get an answer. The modem WITH
#   the SIM (4_1) is left completely alone.
#
#   NOTE: "2_1" is hardcoded for THIS physical unit. Before this firmware
#   goes out to other customer units, confirm their SIM is fitted in the
#   same physical slot — otherwise this would need to target "4_1" instead
#   on units wired the other way round.
#
...
# the real v3.11.16, not the literal placeholder.
if [ -f "$FIRMWARE_BUILD/rocket-dashboard.cgi" ]; then
...
echo "  # apart, since the watchdog now re-checks every 60s forever."
echo "  uptime"
echo "  cat /etc/rocket-routers-version"
echo "  # Should show 2_1 forced disabled every ~60s, no AT/CPIN lines at"
echo "  # all this time (there's nothing left to ask the modem):"
echo "  logread | grep rocket-sim-check"
echo "  uci show qmodem | grep -E 'state|enable_dial'"
echo "  # 2_1 should now read state='disabled' enable_dial='0'."
echo "  # 4_1 (the modem with the SIM) should be untouched: state='enabled' enable_dial='1'."
...
echo ""
echo -e "${YELLOW}${BOLD}NOTE:${RESET} this build does NOT try to detect anything — it just"
echo "force-disables qmodem section 2_1 every 60s, unconditionally. If 2_1"
echo "is still 'enabled'/'1' after a few minutes, the watchdog itself isn't"
echo "starting (init.d/boot order problem) — check 'logread | grep rocket-"
echo "sim-check' is non-empty at all first. If 2_1 IS disabled but the"
echo "reboot loop or no-internet problem persists, that means the SIM-less"
echo "modem was never the only cause of the loop/no-internet — there's"
echo "something else going on, and we'd need to look at that separately."
echo "Also worth testing from a phone/laptop on the router's WiFi, not just"
echo "ping from the router's own SSH session — multi-WAN routing setups"
echo "often can't ping out from the router itself even when client devices"
echo "behind it have full internet."
